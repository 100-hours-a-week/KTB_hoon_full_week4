...
public class MemberController {
    ...
    public ResponseEntity<ApiResponse<Void>> signup(@Valid @RequestBody SignupReqDto request) {
        ...
        return ResponseEntity.status(CREATED.getHttpStatus()).body(ApiResponse.success(CREATED));
    }

    @GetMapping("/profile")
    public ResponseEntity<ApiResponse<MemberProfileResDto>> getProfile(
    ...
    public ResponseEntity<ApiResponse<Void>> updateProfile(
            @LoginMember AuthMember authMember, @Valid @RequestBody ProfileUpdateReqDto request) {
        memberService.updateMemberProfile(authMember.memberId(), request);
        return ResponseEntity.status(SUCCESS.getHttpStatus()).body(ApiResponse.success(SUCCESS));
    }
