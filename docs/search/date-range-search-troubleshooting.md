...
**약 60배.** 그리고 과거 구간과 최근 구간의 차이가 사라졌다(0.105 vs 0.081). 원래 50배 벌어지던 값이라,
"얼마나 과거를 조회하는가"가 더 이상 비용에 영향을 주지 않는다는 뜻이다.
...
8절에서 확인한 900,040행 스캔이 재현되지 않는다.

우려했던 `(:cursorCreatedAt IS NULL OR ...)` 패턴도 파라미터 바인딩 상태에서 문제없이 풀렸다.
...
유닛 테스트로만 보던 누락·중복 없음을 운영 데이터에서도 확인했다.
...
**`over (...)` 가 붙었다.** 커서 위치로 점프했고 11행만 읽었다.
같은 깊이에서 `id` 커서를 쓰면 900,040행 / 7,019ms 이므로 **약 4,500배**다.
우려했던 `(:cursorCreatedAt IS NULL OR ...)` 패턴도 range 로 정상 변환됐다.
...
`ONLINE` + `sido` 는 **결과가 절대 안 나오는 조합**이다. 온라인 모임은 주소가 `null` 이라
`api-specification.md` 에도 그렇게 적혀 있다. MySQL 은 그걸 모르니 100만 행을 다 걷고 나서야 0건이라고 답한다.

그리고 이 조합은 **UI 에서 두 번 클릭하면 만들어진다.** "온라인" 체크 + "서울" 선택. 실수도 공격도 아니다.

### `LIMIT` 이 무력해지는 세 번째 경우
...
**원래 있었는데 안 재봐서 몰랐던 것**이고, 필터 컬럼에 인덱스가 없다는 사실을 한 번도 검토하지 않은 결과다.

### 해볼 만한 것

- **모순 조합을 API 에서 막기.** `meetingType=ONLINE` 인데 `sido`/`sigungu` 가 오면 막거나 무시한다.
  이미 명세에 적힌 사실이라 검증으로 옮기기만 하면 된다. 다만 `ETC` + 특정 지역 + `CLOSED` 처럼
  **논리적으로는 가능한데 데이터가 0건**인 조합은 여전히 남는다.
- **쿼리 타임아웃**(`MAX_EXECUTION_TIME`). 근본 해결은 아니지만 8.7초 동안 커넥션을 물고 있는 상태는 없앤다.
  키워드 34초에도 같이 적용된다.
...
- 결과가 0건인 필터 조합의 실제 사용 빈도. 12절에서 최악 케이스는 확인했지만,
  그런 조합이 얼마나 자주 요청되는지는 모른다

### 11절에서 해소된 것

| 항목 | 결과 |
|---|---|
| 복합 커서가 인덱스를 타는지 | 60페이지까지 응답 시간이 안 늘어 확인 |
| `(:cursorCreatedAt IS NULL OR ...)` 패턴 | 파라미터 바인딩 상태에서도 문제없음 |
| 코드 반영 후 HTTP 응답 시간 | 6.39초 → 0.105초 |
| 조건 없는 목록 17.4ms 가 cold 였는지 | HTTP 0.092초로 정상. cold 측정이었던 것으로 보인다 |
| 커서 경로 테스트 | `SearchServiceTest` 9개 추가. 페이지 끝까지 넘겨 누락·중복이 없는지 본다 |
| 운영 실행 계획 확인 | 12절에서 `EXPLAIN ANALYZE` 로 `over (...)` seek 확인 |
| 필터가 붙은 깊은 커서 | 12절에서 측정. 예상과 달라 별도 절로 정리 |
