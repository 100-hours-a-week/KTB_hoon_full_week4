...
> 이 직접 호출 구조는 다음 증분에서 **트랜잭셔널 아웃박스로 교체됐다** — 위 어긋남 창을
> 닫은 과정은 `search-index-outbox.md`. 아래 7절의 동기화 E2E 는 교체 전 구조의 기록이다.
...
- ~~DB 와 색인의 정합성 검증·재시도 장치는 없다~~ → **트랜잭셔널 아웃박스로 해소했다.**
  여기 적어뒀던 구상(post_id 만 저장, 폴러가 처리 시점 재조회, retry/FAILED)을 그대로
  구현한 기록이 `search-index-outbox.md` 다. 대량 어긋남의 최후 수단으로
  `03_initial_load_ec2.sh` 재적재(멱등)는 여전히 유효하다.
