...
| 키워드 | 매치 건수 | FULLTEXT | LIKE (같은 환경 실측) |
|---|---|---|---|
| `테니스` (매치 4.4%) | 41,679 | **34초** | **33.5ms** |
| `포핸드` | 3,478 | 0.15초 | — |
| `타이브레이크` | 99 | 0.10초 | — |
| 매치 없음 | 0 | 0.06초 | **13,130ms** |
...
반대로 `LIKE`는 인덱스가 주는 순서를 그대로 걷다가 **339행에서 조기 종료**합니다. **두 방식의 최악 케이스가 정반대**라 — `LIKE`는 매치가 없을 때 94만 행을 훑고, FULLTEXT는 매치가 많을 때 무너집니다 — 한쪽으로 되돌리는 건 문제를 옮기는 것에 가깝습니다.

`LIMIT` 축소, 필터 추가, 행 페치 제거, 커서 범위 제한, 날짜 범위 결합, `FORCE INDEX`까지 **MySQL 안에서 여섯 가지 우회를 시도해 전부 막혔습니다.** 특히 `FORCE INDEX`는 `ERROR 1191`로 문법 차원에서 거부됩니다 — `MATCH`를 쓰면 FULLTEXT 인덱스가 접근 경로여야만 하기 때문입니다. 측정 근거와 선택지 정리는 `docs/search/fulltext-search-experiment.md`에 남겼고, 아직 결론을 내지 않았습니다.
