...
- **키워드 정제** : 사용자 입력의 `"` 는 제거합니다(`sanitizeKeyword`). phrase 구문이 중간에 닫혀 쿼리 의미가 깨지는 걸 막기 위해서고, 따옴표 안에서는 `+ - * ~ ( )` 같은 boolean 연산자가 리터럴로 취급되므로 연산자 주입도 함께 차단됩니다. 다만 `ngram_token_size` 가 2 라 **1글자 키워드는 토큰이 만들어지지 않아 매치되지 않습니다** — `LIKE` 시절과 달라진 지점입니다. 이 설정값이 뒤에서 성능까지 가르게 됩니다.
...
**전환 후 측정에서 드러난 한계 (미해결)** — 운영 100만 건 기준으로 다시 재보니, 일부 키워드에서 FULLTEXT가 크게 느렸습니다. 아래는 전부 mysql이 찍어주는 **실제 소요 시간**이고, 캐시 영향이 커서(4.45초 → 0.13초) 2회차 안정값을 씁니다.

| 키워드 | 토큰 | 매치 건수 | FULLTEXT | LIKE (같은 환경) |
|---|---|---|---|---|
| `포핸드` | `[포핸][핸드]` 2개 | 3,478 | 0.13초 | — |
| `라켓` | `[라켓]` **1개** | **69,237** | **0.20초** | — |
| `복식` | `[복식]` **1개** | 49,685 | 0.32초 | — |
| `테니스` | `[테니][니스]` 2개 | 41,679 | **23초** | **33.5ms** |
| 매치 없음 | — | 0 | 0.013초 | **13,130ms** |

**매치 건수가 원인이 아닙니다.** 매치가 가장 많은 `라켓`(69,237건)이 가장 빠르고, 그보다 적은 `테니스`(41,679건)가 23초입니다. 지배 변수는 **검색어가 몇 개 토큰으로 쪼개지느냐**였습니다.

`ngram_token_size=2`라 **2글자 검색어는 토큰이 하나**가 됩니다. 토큰이 하나면 목록만 읽으면 끝이라 매치 건수와 사실상 무관하고, 둘 이상이면 목록을 교차시킨 뒤 **글자 위치가 인접한지까지 대조**해야 해서 매치 건수에 초선형으로 비싸집니다.

여기서 `ngram_token_size`를 1로 낮춰 1글자 검색을 살리면 **모든 다글자 검색어가 다토큰이 되는** 맞바꿈이 생깁니다(추론, 미측정).

그리고 **`ORDER BY`를 제거해도 16초입니다.** FULLTEXT는 매치 집합을 먼저 통째로 만들고 나서 행을 흘려보내기 때문에 `LIMIT`이 11이든 1이든 앞단 비용이 같습니다. "최신순을 포기하면 해결된다"는 선택지가 없습니다.

반대로 `LIKE`는 인덱스가 주는 순서를 그대로 걷다가 **339행에서 조기 종료**합니다. **두 방식의 최악 케이스가 정반대**라 — `LIKE`는 매치가 없을 때 94만 행을 훑고, FULLTEXT는 토큰이 여럿이면서 매치가 많을 때 무너집니다 — 한쪽으로 되돌리는 건 문제를 옮기는 것에 가깝습니다.

`LIMIT` 축소, 필터 추가, 행 페치 제거, `ORDER BY` 제거, 커서 범위 제한, 날짜 범위 결합, `FORCE INDEX`까지 **MySQL 안에서 시도한 우회가 전부 막혔습니다.** 특히 `FORCE INDEX`는 `ERROR 1191`로 문법 차원에서 거부됩니다 — `MATCH`를 쓰면 FULLTEXT 인덱스가 접근 경로여야만 하기 때문입니다.

> 측정 도구에도 함정이 있었습니다. `EXPLAIN ANALYZE`가 보고하는 노드 시간이 실제와 최대 **234배** 어긋납니다(노드는 108ms, 같은 문장의 실제 소요는 25.29초). InnoDB FULLTEXT는 반복자가 돌기 전에 매치 집합을 만드는데 그 시간이 노드에 잡히지 않기 때문입니다. `rows`는 정확하므로 `time`만 버리면 됩니다. 처음에는 이 숫자를 믿고 "`ORDER BY`가 원인"이라는 결론을 세웠다가, 실제 소요 시간으로 재측정하면서 뒤집었습니다.

전 과정과 폐기한 주장은 `docs/search/fulltext-search-experiment.md`에 남겼고, 아직 결론을 내지 않았습니다.
...
약 60배인데, 그보다 **과거 구간과 최근 구간의 차이가 사라진 것**이 핵심입니다. 원래 50배 벌어지던 값이 오차 범위가 됐고, "얼마나 과거를 조회하는가"가 더 이상 비용에 영향을 주지 않습니다.

커서도 `nextCursor`를 따라 60페이지까지 넘겨 확인했습니다. 첫 요청 146.8ms에서 60페이지 42.2ms로 **깊이에 따른 증가 추세가 없었고**, 600건을 모아 중복·누락이 없는 것도 확인했습니다.
...
고치고 나서도 계속 재다가 **별개 문제를 하나 더 발견**했습니다. 결과가 0건인 필터 조합은 `LIMIT`을 채우지 못해 끝까지 걷습니다. 같은 필터에서 `meetingType`만 `OFFLINE`(740행 / 8ms)에서 `ONLINE`으로 바꾸자 **100만 행 / 8,720ms**가 됐습니다. 온라인 모임은 주소가 `null`이라 지역 조건과 함께 오면 결과가 나올 수 없는데, UI에서 두 번 클릭이면 만들어지는 조합입니다. 키워드 검색 34초와 원리가 같습니다 — **찾을 게 없으면 멈출 근거도 없습니다.** 이번 변경이 만든 회귀는 아니고(이전에도 동일), 아직 해결하지 않았습니다.
